...
// A stream we might actually get: nubblyn's own bucket, a hand-pasted
// substitute, or an IMDb id our own stream addons can go search against.
// "Might" is doing real work here — this is a can-attempt flag, not a
// guarantee — but our own backends have good enough coverage on something
// this mainstream that it's a safe bet rather than a hopeful one.
function hasStream(episode) {
  return Boolean(episode.url || episode.streamUrl || episode.overrideUrl || episode.imdb);
}
