...
// carried on a spin-off episode by default — the one exception is `imdb` (and
// a hand-pasted `overrideUrl`), which this script never writes itself but
// carries forward from the existing file if it's already there, so a mapping
// added by hand for something too small or odd for a script (Wilderness
// Years' specials) survives a rebuild instead of silently disappearing.
//
...
for (const s of SERIES) {
  ...
  for (const r of rows) {
    ...
    // Not written by this script — carried forward from whatever's already in
    // the file, so a hand-added IMDb mapping or substitute URL survives a
    // rebuild instead of silently disappearing.
    const prior = before.find((x) => x.season === season && x.episode === episode);
    if (prior?.imdb) e.imdb = prior.imdb;
    if (prior?.overrideUrl) e.overrideUrl = prior.overrideUrl;
    episodes.push(e);
  }
...
// TMDB or IMDb by default; the exception is \`imdb\` (and a hand-pasted
// \`overrideUrl\`), which this script carries forward from the existing file
// rather than writing itself. Edit the ledger and re-run the script for
// everything else; do not edit those fields here either — edit them in place
// and they'll survive the next rebuild.
//
...
`;
  ...
  const body = episodes.map((e) => {
    const lines = KEYS
      ...
      .map((k) => k === 'imdb'
        ? `  imdb: { id: ${JSON.stringify(e.imdb.id)}, season: ${e.imdb.season}, episode: ${e.imdb.episode} },`
        : `  ${k}: ${typeof e[k] === 'number' ? e[k] : JSON.stringify(e[k])},`);
    return '{\n' + lines.join('\n') + '\n}';
  }).join(',\n');
